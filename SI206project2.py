...
## COMMENT HERE WITH:
## Your name: Morgan Feeney
## Anyone you worked with on this project: went to office hours 
...
def grab_headlines():
    ...
    tags = soup.find_all('div', {'class': "view-most-read"})
    for tag in tags:
        findtexttags= tag.find_all('li')
        for x in findtexttags:
...
def get_umsi_data():
    ...
    for i in range(13):
        ...
        tagsx= soup.find_all('div', {'class': 'field field-name-field-person-titles field-type-text field-label-hidden'})
        for tagx in tagsx:
            titles.append(tagx.text)

    together= zip(names, titles)
    umsi_titles= dict(together)
    return umsi_titles
